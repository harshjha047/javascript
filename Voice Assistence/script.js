...
const enter = () => {
  gsap.to(".reactor", {
    ...
    duration: 0.2,
    filter: "drop-shadow(0px 0px 120px rgb(31, 38, 135))",
  });
};
const leave = () => {
  gsap.to(".reactor", {
...
reactor.addEventListener("keypress", function(event) {
    if (event.key === "Enter") {
      indigator.textContent = "Decoding...";
      recognition.start();
    }
...
const takeCommand = (command) => {
  if (command.includes("search")) {
    ...
    fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=96c01dbd7a9ca735381ec9a4b26c0d52`
    )
